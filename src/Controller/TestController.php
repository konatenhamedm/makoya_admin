...
const INDEX_ROOT_NAME = 'app_test_index';
    ...
    'actions' => [
    'edit' => [
    'url' => $this->generateUrl('app_test_edit', ['id' => $value])
    , 'ajax' => true
    , 'icon' => '%icon% bi bi-pen'
    ...
    'show' => [
    'url' => $this->generateUrl('app_test_show', ['id' => $value])
    , 'ajax' => true
    , 'icon' => '%icon% bi bi-eye'
    ...
    'url' => $this->generateUrl('app_test_delete', ['id' => $value])
    , 'ajax' => true
...
'method' => 'POST',
'action' => $this->generateUrl('app_test_new')
]);
...
$isAjax = $request->isXmlHttpRequest();
    ...
    if ($form->isSubmitted()) {
    $response = [];
    $redirect = $this->generateUrl('app_test_index');
...
$form = $this->createForm(Test1Type::class, $test, [
'method' => 'POST',
'action' => $this->generateUrl('app_test_edit', [
'id' => $test->getId()
])
]);
...
return $this->renderForm('test/delete.html.twig', [
'test' => $test,
'form' => $form,
]);
}
